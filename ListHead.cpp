...
void ListHead::Draw(SkCanvas* canvas)
{
	...
	{
		SkRect bounds;
	    font.measureText(columns[k].colname.c_str(), columns[k].colname.size(), kUTF8_SkTextEncoding, &bounds);
		...
		surfaceCanvas->drawSimpleText(columns[k].colname.c_str(), nDrawTextLength, kUTF8_SkTextEncoding, offset_x + ins_x+left_x, 15, font, paint);
